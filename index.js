...
app.get('/allProfiles', isAuthenticated, async function(request, response, next) {
  console.log("Server recieved a get post request at", request.url)
  let results = await dbo.getAllProfiles();
...
app.post('/store', isAuthenticated, async function(request, response, next) {
  ...
  let userId = request.user.userData;
...
app.get('/reminder', isAuthenticated, async function(request, response, next) {
  ...
  let result = await dbo.get_most_recent_planned_activity_in_range(0, currTime, userId)
  await dbo.delete_past_activities_in_range(0, currTime, userId);
...
app.get('/week', isAuthenticated, async function(request, response, next) {
  ...
  console.log("results for speci", result);
...
app.get('/name', isAuthenticated, async function(req, res, next) {
  ...
  let result = await dbo.getFirstName(userId);
...
function gotProfile(accessToken, refreshToken, profile, done) {
  ...
  console.log("Inserted userid: ", userid, "and first name: ", firstName, "into profile table!");

  dbo.insertIntoProfileTable(userid, firstName);
